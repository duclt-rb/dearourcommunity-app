// Nội dung TIẾNG VIỆT của toolkit Năng lượng — bản gốc (source of truth cho migration
// localStorage v1→v2: map nhãn→id build từ chính config này). Bản dịch EN: energy.data.en.ts.
import type {
  AssessmentGroup,
  AssessmentQuestion,
  ActionItem,
...
const FNB: EnergyToolkitConfig = {
  ...
  planFields: [
    { id: 'business-name', label: 'Doanh nghiệp' },
    { id: 'overall-owner', label: 'Người phụ trách chung' },
...
export const ENERGY_TOOLKITS_VI: Record<string, EnergyToolkitConfig> = {
  [FNB.id]: FNB,
  [SUPPLY.id]: SUPPLY,
};
