...
import { ENERGY_TOOLKITS_EN } from './energy.data.en';
import { ENERGY_TOOLKITS_VI } from './energy.data.vi';
...
describe('energy data VI/EN structure parity', () => {
  it('both editions expose the same config ids', () => {
    expect(Object.keys(ENERGY_TOOLKITS_EN).sort()).toEqual(Object.keys(ENERGY_TOOLKITS_VI).sort());
  });

  for (const [id, vi] of Object.entries(ENERGY_TOOLKITS_VI)) {
    it(`config '${id}' has an EN edition with identical structure`, () => {
      const en = ENERGY_TOOLKITS_EN[id];
