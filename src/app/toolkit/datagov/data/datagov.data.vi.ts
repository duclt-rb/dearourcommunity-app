...
const DATA_MAP_COLUMNS: DataMapColumn[] = [
  { key: 'source', label: 'Nguồn thu thập' },
  { key: 'purpose', label: 'Mục đích xử lý' },
  { key: 'legal-basis', label: 'Cơ sở pháp lý' },
  { key: 'sensitive', label: 'Nhạy cảm?' },
  { key: 'storage', label: 'Nơi lưu trữ' },
  { key: 'access', label: 'Ai có quyền truy cập' },
  { key: 'retention', label: 'Thời gian lưu' },
  { key: 'sharing', label: 'Chia sẻ / Bên thứ ba' },
  { key: 'transfer', label: 'Chuyển ra nước ngoài?' },
];
...
/** Bản tiếng Việt — cũng là bản GỐC cho migration persist v1→v2 (map theo nhãn cột VN). */
export const DATAGOV_TOOLKITS_VI: Record<string, DataGovToolkitConfig> = {
